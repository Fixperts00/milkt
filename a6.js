...
const tabImageCounts = [5, 22]; // 첫 번째 탭: 5개, 두 번째 탭: 22개
...
const stepDescriptions = {
  ...
  1: [ // 런처 설치 (22개)
    ...
    "4. 주소창에 밀크T런처 다운로드 주소 url.kr/5zqumj를 입력합니다.",
    "5. '연결이 비공개로 설정되어 있지 않습니다' 라는 창이 노출되면 '고급'을 터치합니다. 이후 'url.kr(안전하지 않음)(으)로 이동'을 터치합니다.",
    "6. 크롬 앱을 닫고 메인 화면으로 이동하여 내 파일 앱을 실행합니다.",
    "7. 내 파일 앱에서 밀크T 런처 apk 파일을 터치합니다.",
    "8. 설치가 차단되었다는 알림창이 뜬다면 '설정'을 터치합니다.",
    "9. 출처를 알 수 없는 앱의 버튼을 터치하여 활성화합니다.",
    "10. '허용'을 터치합니다.",
    "11. '설치'를 터치하여 밀크T 홈을 설치합니다.",
    "12. 밀크T 홈 설치가 완료되면 '열기'를 터치합니다.",
    "13. 아래의 권한을 허용하는 디바이스 관리자를 실행합니다.",
    "14. '허용'을 눌러 밀크T 홈의 파일 액세스 권한을 허용합니다.",
    "15. '허용'을 눌러 밀크T 홈의 위치 액세스 권한을 허용합니다.",
    "16. 가입한 서비스를 선택한 후 '다음'을 터치합니다.",
    "17. 와이파이 연결 확인, 배터리 상태 확인이 완료되면 '다음'을 터치합니다.",
    "18. 개인정보 수집, 이용동의서의 약관동의를 체크 후 확인을 터치합니다.",
    "19. 태블릿 시리얼 인증이 완료된 후 '다음'을 터치합니다.",
    "20. '설치 준비중'이 '설치하기'로 변경되면 터치합니다.",
    "21. 아래의 권한을 허용하는 디바이스 관리자를 실행합니다.",
    "22. 학습에 필요한 콘텐츠를 설치하는데는 상황에 따라 20분~50분 가량 소요됩니다. 콘텐츠 설치가 완료되면 '시작하기'를 터치하여 학습 이용 가능합니다."
  ]
};
...
const additionalNotes = {
  1: { // 런처 설치
    ...
    4: "<span style='color: #dc3545; font-weight: bold;'>실제 보안상에 문제가 있는 것은 아니며, 웹 주소로 이동할 때 노출되는 정상적인 화면입니다.</span> '이러한 형식의 파일은 기기를 손상시킬 수 있습니다. MilkTLauncher.apk을(를) 다운로드하시겠습니까? 창에서 '확인'을 터치하여 파일을 다운로드해주세요.",
    7: "'취소'와 '설정'이 보이지 않고 '확인'만 노출될 경우, 패밀리링크 앱에 의해 설치가 차단된 상황입니다. 학부모님의 휴대폰 앱에서 감독중지를 요청하여 학습생을 감독 대상에서 제외시켜 주세요. 이후 내 파일 앱에서 밀크T 런처 apk 파일을 터치하면 설치 진행 가능합니다.<br><span style='color: #dc3545; font-weight: bold; background-color: #fff3cd; padding: 2px 4px; border-radius: 3px;'>*패밀리링크의 감독 중지 방법은 밀크T에서 안내가 어렵습니다. 만약 감독 중지 진행이 어려우신 경우 구글 측으로 문의해주시기를 부탁드립니다.</span>",
    12: "밀크T 학습을 위한 필수 권한을 허용하는 것으로, 임의로 학습 데이터를 삭제하거나 기기 설정을 변경하는 것은 아닙니다.",
    15: "학습생의 실제 학년에 맞는 서비스를 선택해주세요.(예를 들어, 학습생이 6학년이라면 밀크T 초등 선택)",
    16: "'다음'이 보이지 않을 경우 와이파이 연결이 되지 않았거나 배터리가 30% 미만인 상황입니다. 충전기를 연결하면 배터리 30%가 되지 않더라도 '다음'버튼이 활성화됩니다.",
    18: "인증이 너무 오래 걸리거나 실패했다고 나온다면 와이파이 공유기 재부팅을 부탁드립니다.",
    20: "밀크T 학습을 위한 필수 권한을 허용하는 것으로, 임의로 학습 데이터를 삭제하거나 기기 설정을 변경하는 것은 아닙니다.",
    21: "밀크T와 함께해주셔서 감사합니다. 만약 설치 진행이<br>어려우시거나 정상적으로 설치가 되지 않는 경우에는,<br><span style='color: #dc3545; font-weight: bold; display: block;'>담당 선생님 혹은 밀크T 고객센터로 연락 부탁드립니다.</span>"
  }
};
...
function getImageSrc(device, tabIndex, slideIndex) {
  if (device === 'a6' && tabIndex === 0) {
    ...
    // A6 런처 설치 (22장)
    if (slideIndex === 5) {
      // 5번 이미지는 adimage 레포의 새로운 이미지 사용
      return 'https://raw.githubusercontent.com/skyhigh79/adimage/main/s6_launcher_install_ad.png';
    } else if (slideIndex === 6) {
      // 6번 이미지는 adimage 레포의 새로운 이미지 사용
      return 'https://raw.githubusercontent.com/skyhigh79/adimage/main/a6_launcher_install_ad2.png';
    } else if (slideIndex >= 7) {
      // 7번부터는 원래 이미지 번호에서 1을 뺀 이미지 사용
      const originalIndex = slideIndex - 1;
...
const additionalNotes = {
  1: { // 런처 설치
    ...
    20: "밀크T와 함께해주셔서 감사합니다. 만약 설치 진행이<br>어려우시거나 정상적으로 설치가 되지 않는 경우에는,<br><span style='color: #dc3545; font-weight: bold; display: block;'>담당 선생님 혹은 밀크T 고객센터로 연락 부탁드립니다.</span>"
  }
};
